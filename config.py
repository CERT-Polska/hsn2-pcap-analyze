...
class Config():
	...
	def getConfig(self):
		...
		try:
			ret = self.config.readfp(open("/etc/hsn2/pcap-analyze/pcap-analyze.conf"))
		except IOError:
			logging.warn("Cannot open '/etc/hsn2/pcap-analyze/pcap-analyze.conf'. Exiting...")
			sys.exit(2)
		return self.config
	...
	def getWhiteList(self):
		...
		try:
			...
			logging.warn("Cannot open '/etc/hsn2/pcap-analyze/pcap-analyze-whitelist.conf'. Exiting...")
			sys.exit(2)
